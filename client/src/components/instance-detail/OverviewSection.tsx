  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-medium text-gray-400">Your Gowa URL is</h3>
        <div className="flex items-center gap-2">
          <code className="flex-1 px-4 py-3 font-mono text-sm text-white bg-gray-800 rounded-lg">
            {proxyUrl}
          </code>
          <CopyButton
            content={proxyUrl}
      ...
      {/* Connection info */}
      <div className="space-y-3">
        <h3 className="text-sm font-medium text-gray-400">Connecting to Your Instance</h3>
        <div className="p-4 font-mono text-sm text-gray-300 bg-gray-800 rounded-lg">
  ...
  headers: {${basicAuthPairs.length > 0 ? `
    'Authorization': '${generateToken(basicAuthPairs[0])}'` : ''}
      ...
      {basicAuthPairs.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-gray-400">Authentication Token</h3>
          {basicAuthPairs.map((pair, index) => (
            <div key={index} className="flex items-center gap-2">
              <code className="flex-1 px-4 py-3 font-mono text-sm text-white bg-gray-800 rounded-lg truncate">
                {generateToken(pair)}
              </code>
              <CopyButton
